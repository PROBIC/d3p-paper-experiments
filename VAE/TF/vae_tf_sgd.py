...
parser.add_argument('-lr', '--learning-rate', default=1.0e-5, type=float, help='learning rate')
...
ds, auxinfo = tfds.load('mnist', split='train', with_info=True)
...
def model(data):
  # binarize data
  data = tfd.Bernoulli(probs=data).sample()
  ...
  samples = make_decoder(prior.sample(1), [28, 28]).mean()
...
if args.no_dp:
    optimize = tf.train.GradientDescentOptimizer(args.learning_rate).minimize(-elbo)
else:
