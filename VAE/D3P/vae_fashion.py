...
def model(batch_or_batchsize, z_dim, hidden_dim, out_dim=None, num_obs_total=None):
    ...
    if is_int_scalar(batch_or_batchsize):
        ...
        assert(jnp.ndim(batch) == 3) # three-channel data
    ...
    with plate('batch', num_obs_total, batch_size):
        ...
        x = sample('obs', dist.Bernoulli(img_loc).to_event(1), obs=batch)
        return img_loc
...
def main(args):
    ...
    (train_init, train_fetch), num_samples = load_dataset('fashion_mnist', batch_size=args.batch_size, split='train', batchifier=train_batchifier)
    (test_init, test_fetch), _ = load_dataset('fashion_mnist', batch_size=args.batch_size, split='test', batchifier=split_batchify_data)
    ...
    if args.no_dp:
        ...
        svi = DPSVI(
            ...
            #dp_scale=0.0, clipping_threshold=10000000.,
